...
public interface PostMapper extends BaseMapper<Post> {
     ...
     */
    @Update("UPDATE post SET ${column} = GREATEST(${column} + #{delta}, 0) WHERE id = #{postId}")
    int incrementCount(@Param("postId") Long postId, @Param("column") String column, @Param("delta") int delta);
}
